/**
 * TODO:
 * 1 auto add x-if and x-for template
 * 2 x-prop shortcut using [prop]
 */
...
async function compileTemplate(
  ...
  for (const node of childNodes) {
    if (node.nodeName === 'style' || node.nodeName === 'link') {
      ...
      this.error(`component "${name}" can not contain <template> tag at top level.`)
...
export default defineComponent("${name}", {
  setup: ${code ? 'comp.default' : 'null'},
  props: ${code ? 'comp.$props || {}' : '{}'},
  template: ${JSON.stringify(template)},
  style: ${JSON.stringify(tailwindcss)},
})
  `.trim()
      }
    },
